...
import { getUniCliCache } from './helper';
import { getMovingComponents } from './core';
import type { IMovingComponents, IReplaceRefList, IDispatchVueOptions } from './types.ts';
...
const outputDir = path.resolve(process.env.UNI_OUTPUT_DIR || '');
...
export function getUsingComponentsMap(
  ...
  for (const name of jsonFileMap.keys()) {
    ...
    const { usingAutoImportComponents = {}, usingComponents = {} } = jsonObj;
    ...
    usingComponentsMap[name] = handleUsingComponents(comps);
    if (!pageSet.has(name)) {
      continue;
    }
  }
